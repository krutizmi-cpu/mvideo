...
with st.sidebar:
    ...
    st.download_button("📥 Скачать шаблон Excel", buffer.getvalue(), "template.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
    ...
    with st.status("🔍 Обработка...") as status:
        for index, row in df.iterrows():
            try:
                ...
                l, w, h, weight = float(row.get('д', 0)), float(row.get('ш', 0)), float(row.get('в', 0)), float(row.get('вес', 0))
                ...
                results.append({
                    ...
                    "Прибыль": round(profit, 2), "Цель Маржа %": target_margin,
